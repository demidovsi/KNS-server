...
class ApiKnesset(threading.Thread):
    ...
    def run(self):
        common.write_log_db('LOAD', 'ApiKnesset', 'Поток скачивания информации по API из КНЕССЕТ загружен')
        while True:
            result = False
            t0 = time.time()
            while not result:
                ...
                if not result:
                    ...
                    while count < 60:
                        ...
                        time.sleep(1)
            common.write_log_db('FINISH', 'ApiKnesset', 'Поток API из КНЕССЕТ работу закончил.'
                                                        ' Ожидание 1 час', td=time.time() - t0)
            time.sleep(3600)
